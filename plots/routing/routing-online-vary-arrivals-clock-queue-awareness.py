...
def simulate_queries_oursol(queries, zeta, lambda_val):
    ...
    while queries or any(gpu_queues.values()) or any(queues.values()):
        ...
        global_clock += query_batch_size / lambda_val
...
def main():
    ...
    lambda_values = np.logspace(-2, 3, num=10)
    for lambda_val in lambda_values:
        ...
        print(f"Average Wait Time for Lambda = {lambda_val} is {average_wait_time}")
        data.append(
            {
                "Zeta": zeta,
                "Lambda": lambda_val,
    ...
    for lambda_val in lambda_values:
        ...
        simulate_queries_round_robin(Q, zeta, lambda_val)
        energy, runtime, accuracy, num7b, num13b, num70b = analyze_results()
        average_wait_time = 0
        for K in models:
            average_wait_time += sum(wait_times[K])
    ...
    for lambda_val in lambda_values:
        ...
        Q = deque(zip(lengths, lengths_outputs, arrival_times))
        lambda_val = num_queries / end_time
        simultate_queries_random(Q, zeta, lambda_val)
        energy, runtime, accuracy, num7b, num13b, num70b = analyze_results()
        average_wait_time = 0
        for K in models:
            average_wait_time += sum(wait_times[K])
    ...
    plt.ylim(0, 1000)
    plt.savefig(
        f"lambda-routing-online-awareness-energy-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    plt.savefig(
        f"lambda-routing-online-awareness-accuracy-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    plt.ylim(0, 800)
    plt.savefig(
        f"lambda-routing-online-awareness-runtime-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    plt.savefig(
        f"lambda-routing-online-awareness-allocation-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    plt.savefig(
        f"lambda-routing-online-awareness-wait-time-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
