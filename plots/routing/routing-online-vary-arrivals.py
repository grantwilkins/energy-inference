...
def handle_query(query, zeta):
    ...
    if len(gpu_queues[selected_model]) < gpu_capacities[selected_model]:
        projected_end_time = arrival_time + runtime_cost(selected_model, t_in, t_out)
        ...
        queues[selected_model].append((t_in, t_out, arrival_time))
...
def handle_query_round_robin(query, current_index):
    t_in, t_out, arrival_time = query
...
def main():
    ...
    # arrival_times = sorted(np.random.uniform(0, 1000, num_queries))
    ...
    gpu_capacities = {
        K: math.floor(int(gamma_K[K] * num_gpus) / n_K[K]) for K in models
    }
    ...
    # end_time_values = np.logspace(1, 4, num=10)
    lambda_values = np.logspace(-2, 3, num=20)
    for lambda_val in lambda_values:
        zeta = 0.0
        end_time = num_queries / lambda_val
        arrival_times = sorted(np.random.uniform(0, end_time, num_queries))
        Q = list(zip(lengths, lengths_outputs, arrival_times))
        simulate_queries_oursol(Q, zeta)
        print(f"Results for Lambda = {zeta}")
    ...
    for lambda_val in lambda_values:
        ...
        arrival_times = sorted(np.random.uniform(0, end_time, num_queries))
        Q = list(zip(lengths, lengths_outputs, arrival_times))
        lambda_val = num_queries / end_time
    ...
    plt.savefig(
        f"lambda-routing-online-energy-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    plt.savefig(
        f"lambda-routing-online-accuracy-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    plt.xscale("log")
    plt.savefig(
        f"lambda-routing-online-runtime-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    plt.savefig(
        f"lambda-routing-online-allocation-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
    ...
    # )
    plt.savefig(
        f"lambda-routing-online-wait-time-{gamma_K['Llama-2 (7B)']}-{gamma_K['Llama-2 (13B)']}-{gamma_K['Llama-2 (70B)']}.pdf",
        bbox_inches="tight",
    )
